...
class PDFOptimizer:
    ...
    def _aggressive_optimization(
        ...
        try:
            ...
            # Calculate pages to keep based on target size
            current_size = len(pdf_content)
            size_ratio = target_size / current_size
            pages_to_keep = max(1, int(len(pdf_document) * size_ratio * 0.8))  # 80% to ensure under limit
            
            # Extract text and create minimal PDF
            text_pdf = fitz.open()
